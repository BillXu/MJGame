...
"AoGAQP4KdnJvYDD0Y4coUxDGevG5AirXix43v1lOkvUia + R + s4KYcHk44dtYx1r9\n"
"Z0covt2fTo7EeYXa + txBCt + zCoodH4LIcXEyQRhCZTomtnyH / SyHYeffmYosqGM4\n"
"SsE + Gsyd / xW7pFHy2dDXHyNtPw6piOCFAYu8xVbzJR2UJ0ECQQDknhWuip95FdHT\n"
"R / OhKviMdOttIkJVhLR4zllssjIkQ9 + wKmzkSOAvOa6abZ4W78sZkB / wbVmWBAsC\n"
"J5pywsSRAkEA4ErCy0uxqDfTc5MAShBKtszmpfep4oWogHWEWCqi8Sz5Ob / HIXTz\n"
"+ 213H / DDrskkRs2bi1cGlSDx0DM7 + E5hlQJAXxtk6bfBoB3zrHHCaMn3nHcHed9G\n"
"uUz7x0YQ7ac0NOIHrMH5iv7GRN86RARWQDSByaCrqNnU6xPDgKH5HsToAQJAa4tt\n"
...
string OpenapiClient::analyzeResponse(const string &responseStr) {
    ...
    if (!alipayPublicKey.empty()) {
        ...
        LOGFMTE("AlipayPublicKey:%s", alipayPublicKey.c_str());

        JsonMap::const_iterator iter = responseMap.find(OpenapiClient::KEY_SIGN);
        if (iter == responseMap.end()) {
            LOGFMTE("Cannot get Sign from response, Verify Failed");
            return string();
        }
        //获取返回报文中的sign;
        string responseSign = iter->second.toString();

        LOGFMTE("ResponseSign:%s", responseSign.c_str());

        //调用验签方法;
        bool verifyResult = OpenapiClient::rsaVerify(responseContent, responseSign, alipayPublicKey);

        if (!verifyResult) {
            LOGFMTE("Verify Failed");
            return string();
        }
        LOGFMTE("Verify Success");
    } else {
        LOGFMTE("AlipayPublicKey is empty, Skip the Verify");
    }
